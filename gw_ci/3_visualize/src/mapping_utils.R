...
plot_gw_static_png <- function(
  ...
  canvas <- grid::rectGrob(
    ...
    gp = grid::gpar(fill = viz_cfg$bg_col, alpha = 1, col = viz_cfg$bg_col)
  )

  p <- ggdraw(ylim = c(0, 1), xlim = c(0, 1)) +
    # a background
    draw_grob(
      canvas,
      x = 0,
    ...
    draw_image(base_img, x = 0, y = 0, width = 1, height = 1) +
    # Mock image legend
    draw_image(
      legend_img,
      x = 0.9,
